...
def _add_id() -> None:
    con = duckdb.connect()
    con.execute("""
        CREATE OR REPLACE TABLE raw_with_id AS
        SELECT *, uuid() AS unique_id
        FROM read_csv_auto('/opt/airflow/data/raw_data.csv', header=True)
    """)
    con.execute("COPY raw_with_id TO '/opt/airflow/data/raw_data.csv' (HEADER, DELIMITER ',')")
    con.close()
...
def _update_datetime() -> None:
    if str(date.today()) != "2025-04-11":
        yesterday_time = datetime.now().replace(microsecond=0) - timedelta(days=1)
        con = duckdb.connect()
        con.execute(f"""
            CREATE OR REPLACE TABLE raw_updated AS
            SELECT
                person_name, user_name, email, personal_number, birth_date, address,
                phone, mac_address, ip_address, iban,
                TIMESTAMP '{yesterday_time}' AS accessed_at,
                session_duration, download_speed, upload_speed, consumed_traffic, unique_id
            FROM read_csv_auto('/opt/airflow/data/raw_data.csv', header=True)
        """)
        con.execute("COPY raw_updated TO '/opt/airflow/data/raw_data.csv' (HEADER, DELIMITER ',')")
        con.close()
